...
import {ILexarDomainFactory} from "./interface/ILexarDomainFactory.sol";
import {ILexarDomainSBT} from "./interface/ILexarDomainSBT.sol";
import {ILexarDomainHub} from "./interface/ILexarDomainHub.sol";
import "./lib/strings.sol";
import "@openzeppelin/contracts/utils/Strings.sol";
import "@openzeppelin/contracts/token/ERC721/extensions/IERC721Metadata.sol";

contract LexarSBTDomainResolver is Initializable, OwnableUpgradeable {
  ...
  function getDefaultDomain(address _addr, string calldata _tld) public view returns(string memory) {
    ...
    for (uint256 i = 0; i < fLength;) {
      address tldAddr = ILexarDomainFactory(factories[i]).tldNamesAddresses(_tld);

      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        return string(ILexarDomainSBT(tldAddr).defaultNames(_addr));
      }
  ...
  function getDefaultDomains(address _addr) public view returns(string memory) {
    ...
    uint256 fLength = factories.length;
    for (uint256 i = 0; i < fLength;) {
      string[] memory tldNames = ILexarDomainFactory(factories[i]).getTldsArray();

      for (uint256 j = 0; j < tldNames.length; ++j) {
        string memory tldName = tldNames[j];
        address tldAddr = ILexarDomainFactory(factories[i]).tldNamesAddresses(tldName);
        string memory defaultName = ILexarDomainSBT(tldAddr).defaultNames(_addr);
  ...
  function getDomainHolder(string calldata _domainName, string calldata _tld) public view returns(address) {
    ...
    for (uint256 i = 0; i < fLength;) {
      ...
      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        return address(ILexarDomainSBT(tldAddr).getDomainHolder(_domainName));
      }
  ...
  function getDomainData(string calldata _domainName, string calldata _tld) public view returns(string memory) {
    ...
    for (uint256 i = 0; i < fLength;) {
      ...
      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        return string(ILexarDomainSBT(tldAddr).getDomainData(_domainName));
      }
  ...
  function getDomainTokenUri(string calldata _domainName, string calldata _tld) public view returns(string memory) {
    ...
    for (uint256 i = 0; i < fLength;) {
      ...
      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        (, uint256 _tokenId, , ) = ILexarDomainSBT(tldAddr).domains(_domainName);
  ...
  function getTldAddress(string calldata _tldName) public view returns(address) {
    uint256 fLength = factories.length;
    for (uint256 i = 0; i < fLength;) {
      address tldAddr = ILexarDomainFactory(factories[i]).tldNamesAddresses(_tldName);
  ...
  function _isHubAdmain() internal {
        require(ILexarDomainHub(hubAddress).checkHubAdmin(_msgSender()), "Not Hub Admin");
    }
}