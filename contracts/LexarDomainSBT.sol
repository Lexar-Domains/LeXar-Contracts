...
import { ILexarDomainMetadata } from "./interface/ILexarDomainMetadata.sol";
...
/// @title Lexar Domain contract
/// @author Ademola Adebowale
/// @notice Dynamically generated NFT Domain Contract
contract LexarDomainSBT is ILexarDomainSBT, ERC4973, Ownable, ReentrancyGuard {
  using strings for string;

  // Domain struct is defined in ILexarDomain
  address public metadataAddress; // Lexar Domain metadata contract address
  ...
  function tokenURI(uint256 _tokenId) public view override returns (string memory) {
    return ILexarDomainMetadata(metadataAddress).getMetadata(
  ...
  function _mintDomain(
    ...
    Domain memory newDomain; // Domain struct is defined in ILexarDomain
  ...
  function _sendPayment(uint _amount) internal {
    (address addr, uint percentage) = ILexarDomainFactory(factoryAddress).getRoyaltyDetails();
